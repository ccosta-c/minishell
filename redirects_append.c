...
int	redi_append(t_data *data, char *str, char *next_s)
{
	...
	if (str[0] == '>' && str[1] == '>' && !str[2])
	{
		if (redi_out_search(next_s, 0, 0) == 0)
		{
			...
			if (redi_append_two_nodes(data, str, tmp) == -1)
				return (free(tmp), -1);
			return (free(tmp), 0);
		}
		else if (redi_out_search(next_s, 0, 0) != 0)
		{
			tmp = cut_str_append(next_s, data, -1, 0);
			if (redi_append_two_nodes_cut(data, str, tmp) == -1)
				return (free(tmp), -1);
			return (free(tmp), 0);
		}
	}
...
int	redi_append4(t_data *data, char *str)
{
	...
	if (str[0] == '<' && str[1] == '<' && str[2])
	{
		if (redi_out_search(str, 2, 0) == 0)
		{
			...
			if (redi_append_one_node_del(data, str, tmp) == -1)
				return (free(tmp), -1);
			return (free(tmp), 0);
		}
		else if (redi_out_search(str, 2, 0) != 0)
		{
			tmp = cut_str_1_append(str, data, -1, 2);
			if (redi_append_no_node(data, tmp) == -1)
				return (free(tmp), -1);
			return (free(tmp), 0);
		}
	}
...
int	redi_append5(t_data *data, char *str)
{
		...
		&& (str[ft_strlen(str) - 1] != '>' || str[ft_strlen(str) - 1] != '<'))
	{
		...
		{
			tmp = cut_str_else_append(str, data, j, 0);
			if (redi_append_no_node(data, tmp) == -1)
				return (free(tmp), -1);
			return (free(tmp), 0);
		}
...
int	redi_append2(t_data *data, char *str, char *next_s)
{
		...
		&& str[ft_strlen(str) - 2] == '>' && str[ft_strlen(str) - 1] == '>')
	{
		...
		{
			tmp = cut_str_end_append(str, data, 0, next_s);
			if (redi_append_one_node_del(data, next_s, tmp) == -1)
				return (free(tmp), -1);
			return (free(tmp), 0);
		}
		else if (redi_out_search(str, 0, 0) == 1
			&& redi_out_search(next_s, 0, 0) != 0)
		{
			tmp = cut_str_end2_append(str, data, 0, next_s);
			if (redi_append_no_node(data, tmp) == -1)
				return (free(tmp), -1);
			return (free(tmp), 0);
		}
...
int	redi_append3(t_data *data, char *str)
{
		...
		&& (str[ft_strlen(str) - 1] == '>' || str[ft_strlen(str) - 1] == '<'))
	{
		if (redi_out_search(str, 0, 0) > 1)
		{
			tmp = cut_str_else_append(str, data, cut_str2(str, 0, 0, 0), 0);
			if (redi_append_no_node(data, tmp) == -1)
				return (free(tmp), -1);
			return (free(tmp), 0);
		}
